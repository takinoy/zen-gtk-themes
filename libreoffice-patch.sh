...
# bash -c 'GTK2_RC_FILES=$HOME/.themes/XZen/gtk-2.0/apps/libreoffice.rc application-command'
...
theme_dir=""
cd /usr/share/themes
if find XZen* > /dev/null
then
theme_dir=$(pwd)/$(find XZen* | head -1)
else
cd $HOME/.themes
	if find XZen* > /dev/null
	then
	...
	echo "Theme XZen* not installed, aborded"
	exit 1
	fi
fi
